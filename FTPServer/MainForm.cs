...
namespace FTPServer
{
    public partial class MainForm : Form
    {
        ...
        {
            if (this.certpathtextBox.Text.Trim() == string.Empty || this.certpwdtextBox.Text.Trim() == string.Empty)
            {
                MessageBox.Show("证书路径或证书密码不能为空！");
                return;
            }
            this._ftpServer.ImportCertificate(this.certpathtextBox.Text.Trim(), this.certpwdtextBox.Text.Trim());
            MessageBox.Show(" 成功输入证书");
        }

        private void lblOpenCert_Click(object sender, EventArgs e)
        {
            OpenFileDialog fbd = new OpenFileDialog();
            fbd.Title = "请选择.pfx证书文件";
            fbd.AddExtension = true;
            fbd.Filter = "pfx files (*.pfx)|*.pfx";
            if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                this.certpathtextBox.Text = fbd.FileName;
            }
        }
    }
}
